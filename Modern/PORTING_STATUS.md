...
- E-mote PSB discovery, candidate-key parsing, chunk listing, and extraction are available through the modern port with its migrated 13-key list.
- Leaf AM discovery, encrypted index parsing, and content extraction are available through the modern port with its migrated 65,536-byte decrypt table.
- Lucifen LPK discovery, encrypted index parsing, and entry extraction are available through the modern port with its migrated 19-scheme and 22-title file-key maps.
- ExHIBIT GYU metadata and uncompressed pixel extraction are available through the modern port with its migrated seven numeric-title and two string-title key maps.
- YuRis YPF discovery, title-bound directory decoding, and entry extraction are available through the modern port with its migrated 82-scheme map.
...
- DPK discovery, title/basename scheme lookup, and encrypted entry extraction are available through the modern port with its migrated nine-scheme array.
- PAK/AGSI discovery, title/archive key lookup, and encrypted DES entry extraction are available through the modern port with its migrated ten-title, 103-key nested map.
- PAK/LEAF discovery, title-key lookup, and encrypted entry extraction are available through the modern port with its migrated six-key map.
...
- YPF entries using the legacy Snappy compression mode are rejected until a cross-platform Snappy dependency is selected; zlib and uncompressed entries remain supported.
...
- XP3 now bundles 431 checksum-verified v2 profiles, including Hx, Senren, related Cx helpers, 228 simple hash-based records, and key-parameterized Akabei/Mado/Xor/Stripe/Smile/SMX profiles. Migrated executable bindings are available for automatic XP3 detection; 21 list-file/key-table profiles and real archive fixture coverage are still pending.
