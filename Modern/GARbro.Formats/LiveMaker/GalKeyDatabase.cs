...
namespace GameRes.Formats.LiveMaker
{
    ...
    static class GalKeyDatabase
    {
        ...
        {
            var data = GameRes.Formats.GameDataCatalog.LoadDataset<GalKeyData> ("gal-keys");
            if (data.SchemaVersion != SchemaVersion || data.KnownKeys == null || data.KnownKeys.Count == 0)
                throw new InvalidDataException ("GAL key dataset is invalid.");
            ...
            foreach (var item in data.KnownKeys)
            {
                if (string.IsNullOrWhiteSpace (item.Key) || string.IsNullOrEmpty (item.Value)
                    || item.Value.Length < 4)
                    throw new InvalidDataException ("GAL key dataset contains an incomplete entry.");
                if (!keys.TryAdd (item.Key, item.Value))
                    throw new InvalidDataException ("GAL key dataset contains a duplicate title: " + item.Key);
            }
            return keys;
        }
    }
