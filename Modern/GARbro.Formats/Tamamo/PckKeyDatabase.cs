...
namespace GameRes.Formats.Tamamo
{
    ...
    static class PckKeyDatabase
    {
        ...
        {
            var data = GameRes.Formats.GameDataCatalog.LoadDataset<PckKeyData> ("pck-keys");
            if (data.SchemaVersion != SchemaVersion || data.KnownKeys == null || data.KnownKeys.Count == 0)
                throw new InvalidDataException ("PCK key dataset is invalid.");

            var keys = new Dictionary<string, byte[]> (StringComparer.OrdinalIgnoreCase);
            foreach (var item in data.KnownKeys)
            {
                if (string.IsNullOrWhiteSpace (item.Key) || item.Value == null
                    || item.Value.Length == 0 || item.Value.Length > 56)
                    throw new InvalidDataException ("PCK key dataset contains an incomplete entry.");
                if (!keys.TryAdd (item.Key, item.Value))
                    throw new InvalidDataException ("PCK key dataset contains a duplicate title: " + item.Key);
            }
            return keys;
        }
    }
