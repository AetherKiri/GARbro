...
namespace GameRes.Formats.GameSystem
{
    ...
    static class CmpKeyDatabase
    {
        ...
        {
            var data = GameRes.Formats.GameDataCatalog.LoadDataset<CmpKeyData> ("cmp-keys");
            if (data.SchemaVersion != SchemaVersion || data.KnownKeys == null || data.KnownKeys.Count == 0)
                throw new InvalidDataException ("CMP key dataset is invalid.");
            ...
            foreach (var item in data.KnownKeys)
            {
                if (string.IsNullOrWhiteSpace (item.Key) || item.Value == null || item.Value.Length == 0)
                    throw new InvalidDataException ("CMP key dataset contains an incomplete entry.");
                if (!keys.TryAdd (item.Key, item.Value))
                    throw new InvalidDataException ("CMP key dataset contains a duplicate title: " + item.Key);
            }
            return keys;
        }
    }
