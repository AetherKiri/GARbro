...
namespace GameRes.Formats.Mg
{
    ...
    static class MgKeyDatabase
    {
        ...
        {
            var data = GameRes.Formats.GameDataCatalog.LoadDataset<MgKeyData> ("mgpk-keys");
            if (data.SchemaVersion != SchemaVersion || data.KnownKeys == null || data.KnownKeys.Count == 0)
                throw new InvalidDataException ("MGPK key dataset is invalid.");
            var keys = new Dictionary<string, byte[]> (StringComparer.Ordinal);
            foreach (var item in data.KnownKeys)
            {
                if (string.IsNullOrWhiteSpace (item.Key) || item.Value == null || item.Value.Length == 0)
                    throw new InvalidDataException ("MGPK key dataset contains an incomplete entry.");
                if (!keys.TryAdd (item.Key, (byte[])item.Value.Clone ()))
                    throw new InvalidDataException ("MGPK key dataset contains a duplicate title: " + item.Key);
            }
            return keys;
        }
    }
