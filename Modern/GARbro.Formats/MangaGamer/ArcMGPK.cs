...
{
    internal sealed class MgArchive : ArcFile
    {
    ...
    public class MgpkOpener : ArchiveFormat
    {
        ...
        static void DecryptData (byte[] input, byte[] key)
        {
            key = (byte[])key.Clone ();
        ...
        internal static Stream DecompressStream (byte[] input)
        {
            byte[] output = new byte[Math.Max (input.Length * 2, 1)];
            int output_size = LzfDecompress (input, ref output);
            return new BinMemoryStream (output, 0, output_size);
        }

        static int LzfDecompress (byte[] input, ref byte[] output)
        {
            int src = 0;
            int dst = 0;
        ...
        protected static byte[] QueryKey (string archiveName)
        {
            var title = FormatCatalog.Instance.LookupGame (archiveName);
            if (string.IsNullOrEmpty (title))
                title = FormatCatalog.Instance.LookupGame (archiveName, @"..\*.exe");
            return GetKey (title);
        }
        ...
        static MgScheme DefaultScheme = new MgScheme { KnownKeys = MgKeyDatabase.CreateSchemeKeys () };
